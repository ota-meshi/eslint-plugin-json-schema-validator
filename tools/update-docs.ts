        ...
        if (deprecated) {
            if (replacedBy) {
                const replacedRules = replacedBy.map(
                    (name) =>
                        `[json-schema-validator/${name}](${name}.md) rule`,
                )
                notes.push(
            ...
            if (categories) {
                const presets = []
                // eslint-disable-next-line @typescript-eslint/require-array-sort-compare -- string sort
...
- [Test source](https://github.com/ota-meshi/eslint-plugin-json-schema-validator/blob/main/tests/src/rules/${ruleName}.js)
`
